    ...
    for (vendor_code,original),g in df.groupby(["廠商代碼","原廠編號"],sort=False):
        ...
        for color,cg in g.groupby("顏色",sort=False):
            ...
            records.append({
                ...
                "備註1":note1,"原廠編號":original,"材質四":first.get("備註2", ""),
...
if st.session_state.ocr_df is not None:
    ...
    if brand == "AN":
        ...
        brand_code="3"
        output_brand_code="003"
        with a: st.text_input("品牌代碼（3位）",value=output_brand_code,disabled=True,key="JB_brand_code")
